...
# ============================================================
# 运行环境变量设置（必须在导入 torch 之前）
# ============================================================
import os

# KMP_DUPLICATE_LIB_OK:
#   解决某些环境下（尤其是 Windows / Anaconda）MKL/OpenMP 动态库重复加载导致的报错/崩溃。
os.environ["KMP_DUPLICATE_LIB_OK"] = "TRUE"

# TORCHDYNAMO_DISABLE / TORCH_COMPILE_DISABLE:
#   禁用 TorchDynamo 与 torch.compile()。
#   该脚本包含“结构化深度剪枝”（会在训练后删除 Transformer 层），
#   与编译图缓存机制可能冲突，因此默认关闭以保证稳定性与可复现性。
os.environ["TORCHDYNAMO_DISABLE"] = "1"
os.environ["TORCH_COMPILE_DISABLE"] = "1"

# OMP_NUM_THREADS / MKL_NUM_THREADS:
#   限制 CPU 线程数，避免 DataLoader/NumPy 在多核机器上过度抢占导致性能抖动。
...
# ============================================================
# 标准库 / 第三方库导入
# 说明：已按你的要求移除所有可视化相关依赖（matplotlib/seaborn 等）。
# ============================================================
import dataclasses
import math
import random
...
# 训练脚本中常见的告警较多（例如某些数值范围、性能提示）。
# 如需严格排查问题，可将该行注释掉，以便看到完整 warning。
warnings.filterwarnings("ignore")

# 统一 DDM 字段优先级，避免“统计阶段”和“训练阶段”从不同字段读取导致口径不一致。
# 这些字段名用于兼容不同版本的数据集 pkl（有的保存为 ddm，有的保存为 ddm_tensor 等）。
...
def _install_numpy_core_compat() -> None:
    """
    兼容性补丁：为旧版 NumPy pickle 反序列化注入模块别名。

    一些历史数据（例如某些 .pkl）在序列化时引用过 ``numpy._core`` 这一路径，
    但在新版本 NumPy 中该模块路径可能不存在，导致反序列化时报错：
        ModuleNotFoundError: No module named 'numpy._core'

    处理方法：
        通过向 ``sys.modules`` 注册一个名为 ``numpy._core`` 的模块对象，
        并把 ``numpy.core`` 的符号表复制进去，从而让旧 pickle 可以在新环境中正常加载。
    """
    try:
...
def set_seed(seed: int, deterministic: bool = True) -> None:
    ...
    影响范围：
        ...
        - NumPy
        - PyTorch（CPU 与 CUDA）
        - cuDNN 的确定性/benchmark 开关

    参数：
        seed: 随机种子。
        deterministic:
            True  -> 尽量保证每次运行结果一致（可能牺牲性能）；
            False -> 允许 cuDNN benchmark 自动选择更快算法（结果可能有微小波动）。
    """
    random.seed(seed)
    np.random.seed(seed)
...
def setup_gpu_optimizations(*, deterministic: bool = True) -> None:
    ...
    本函数主要做三件事：
        1) TF32 开关：在支持的 GPU 上可用 TF32 加速矩阵乘（会带来轻微数值误差）。
        2) cuDNN benchmark / deterministic：在“可复现”与“性能”之间做权衡。
        3)（可选）设置进程显存占比，尽量减少 OOM 与显存碎片（不同版本/平台可能不支持）。

    参数：
        deterministic:
            True  -> 追求可复现：关闭 TF32、关闭 benchmark、开启 deterministic；
            False -> 追求性能：开启 TF32、开启 benchmark（结果可能有微小波动）。
    """
...
def compute_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
    """
    计算回归任务的三项核心指标：RMSE / Bias / Corr（皮尔逊相关系数）。

    说明：
        - 会自动过滤 NaN/Inf；
        - Bias 定义为 mean(y_pred - y_true)；
        - Corr 使用中心化后的相关系数形式计算（当方差为 0 时返回 NaN）。
    """
...
def wind_speed_intervals() -> List[Tuple[float, float]]:
    """
    定义风速分区间，用于分段评估模型在不同风速范围内的表现。

    返回：
        List[(lo, hi)]，区间为左闭右开 [lo, hi)。最后一个区间上界为 +inf。
    """
...
def compute_interval_metrics(y_true: np.ndarray, y_pred: np.ndarray) -> List[Dict[str, float]]:
    ...
    返回：
        每个区间一个 dict：包含 lo/hi/n 以及 compute_metrics 的输出。
    """
    y_true = np.asarray(y_true, dtype=np.float64).reshape(-1)
    y_pred = np.asarray(y_pred, dtype=np.float64).reshape(-1)
...
def build_aux_columns(df: pd.DataFrame) -> List[str]:
    """
    构建并返回模型使用的辅助特征列名，同时在 df 中生成必要的派生列。

    该函数会做三类事情：
        1) 计算每条样本的 SP（Specular Point，镜面反射点）在 DDM 网格中的位置索引 ``sp_index``；
           - 数据集中需要提供 ``sp_delay_bin`` 与 ``sp_doppler_bin``；
           - 这里假定 DDM 网格为 11x11，因此展平索引：sp_index = delay_bin * 11 + doppler_bin。
        2) 对部分物理量做数值稳定的变换（例如 log10 / sqrt），缓解长尾分布并提升可学习性。
        3) 选择最终可用的辅助特征列，并进行 NaN 填充与“标签泄露”检查（防止把 WS 等标签当特征）。

    返回：
        cols: List[str]，用于 STGWindowDataset 读取 df[cols] 作为 aux 特征输入。
    """
    preferred = [
...
def evaluate(model: nn.Module, loader: DataLoader, device: str) -> Dict[str, float]:
    """
    在给定 DataLoader 上进行推理并计算回归指标（RMSE / Bias / Corr）。

    关键点：
        - 使用 @torch.no_grad() 禁用梯度，减少显存占用并提升推理速度；
        - 若在 CUDA 上运行，则启用 AMP autocast，使推理精度/性能与训练设置一致；
        - 将所有 batch 的结果收集到 CPU/numpy 后统一计算指标，避免逐 batch 统计带来的误差。
    """
    model.eval()
    ys: List[np.ndarray] = []
    yh: List[np.ndarray] = []
...
def train_one_epoch(
    ...
    """
    训练一个 epoch，并返回该 epoch 的平均 MSE 损失。

    训练步骤：
        1) 从 DataLoader 取出一个 batch（DDM 张量 + aux 特征 + 标签 y + SP 索引）；
        2) 前向计算得到 pred；
        3) 使用均方误差 MSE 作为训练损失；
        4) 反向传播 + 梯度裁剪 + 参数更新：
           - 若 scaler 非空：走 AMP 混合精度路径（scale/unscale/step/update）；
           - 否则：走常规 FP32 路径。
    """
    model.train()
    losses: List[float] = []
...
def _diagnose_rcg_distribution(df: pd.DataFrame, out_dir: Path) -> None:
    """
    诊断 range_corr_gain (RCG) 的数值分布（无绘图版）。

    背景说明：
        RCG（Range Correction Gain）在部分数据集/处理链路中可能存在“口径混杂”问题，
        表现为数值分布呈现明显的双峰或分段现象，进而导致模型在某些风速区间出现异常偏差。

    本函数做什么：
        1) 打印 RCG 及 log10(RCG) 的关键统计量（min/max/mean/median/std/偏度/峰度等）；
        2) 计算一个简单的“双峰倾向”指标（Bimodality Coefficient，经验指标）；
        3) 将统计结果保存为 CSV，方便后续排查（不生成任何图像文件）。

    参数：
        df:      数据集 DataFrame（需包含 range_corr_gain 列）。
        out_dir: 输出目录（CSV 将写入该目录）。
    """
    ...
    if len(rcg) == 0:
        print("⚠️ range_corr_gain 全为空，跳过 RCG 诊断")
        return

    # 基础统计（原始尺度）
    rcg_min = float(rcg.min())
    rcg_max = float(rcg.max())
    rcg_mean = float(rcg.mean())
    rcg_median = float(rcg.median())
    rcg_std = float(rcg.std())

    # 对数尺度更容易观察“分叉/双峰”
    log_rcg = np.log10(rcg.clip(lower=1e-10).to_numpy(dtype=np.float64))
    log_mean = float(np.mean(log_rcg))
    log_std = float(np.std(log_rcg))

    # 计算偏度与峰度（不依赖 scipy）
    # skew  = E[((x-μ)/σ)^3]
    # kurt  = E[((x-μ)/σ)^4] - 3  （这里使用 excess kurtosis）
    if log_std > 0:
        z = (log_rcg - log_mean) / (log_std + 1e-12)
        log_skew = float(np.mean(z ** 3))
        log_kurt_excess = float(np.mean(z ** 4) - 3.0)
        log_kurt = log_kurt_excess + 3.0
        # Bimodality Coefficient (BC)：(skew^2 + 1) / kurtosis
        # 经验上 BC > ~0.555 可能提示双峰倾向（并非严格统计检验）。
        bc = float((log_skew ** 2 + 1.0) / log_kurt) if log_kurt > 0 else float("nan")
    else:
        log_skew = float("nan")
        log_kurt_excess = float("nan")
        bc = float("nan")
    ...
    print("=" * 50)
    print(f"  样本数: {len(rcg)}")
    print(f"  RCG 范围:   [{rcg_min:.4e}, {rcg_max:.4e}]")
    print(f"  RCG 均值/中位数/标准差: {rcg_mean:.4e} / {rcg_median:.4e} / {rcg_std:.4e}")
    print(f"  log10(RCG) 均值/标准差: {log_mean:.4f} / {log_std:.4f}")
    print(f"  log10(RCG) 偏度/峰度(超额): {log_skew:.4f} / {log_kurt_excess:.4f}")
    print(f"  双峰倾向系数 BC(经验): {bc:.4f}  (经验阈值 ~0.555，仅供参考)")

    # rcg_source 常用于标注 RCG 来源/口径（若有该列，输出分布）
    if "rcg_source" in df.columns:
        sources = df["rcg_source"].value_counts(dropna=False)
        print("\n  RCG 来源分布(rcg_source):")
        for src, cnt in sources.items():
            print(f"    {src}: {cnt} ({100 * cnt / len(df):.1f}%)")

        # 保存来源分布，便于离线排查
        out_src = out_dir / "rcg_source_counts.csv"
        sources.rename("count").to_csv(out_src)
        print(f"  已保存 rcg_source 统计: {out_src}")

    # 保存一份汇总统计到 CSV
    out_dir.mkdir(parents=True, exist_ok=True)
    out_csv = out_dir / "rcg_diagnosis.csv"
    pd.DataFrame([
        {
            "n": int(len(rcg)),
            "rcg_min": rcg_min,
            "rcg_max": rcg_max,
            "rcg_mean": rcg_mean,
            "rcg_median": rcg_median,
            "rcg_std": rcg_std,
            "log_mean": log_mean,
            "log_std": log_std,
            "log_skew": log_skew,
            "log_kurtosis_excess": log_kurt_excess,
            "bimodality_coefficient": bc,
        }
    ]).to_csv(out_csv, index=False)
    print(f"  已保存 RCG 诊断统计: {out_csv}")
    print("=" * 50 + "\n")
...
def _diagnose_high_wind_bimodality(
    ...
    """
    高风速区间“分布异常/可能双峰”诊断（无绘图版）。

    思路：
        将真值 y_true 按多个窄风速区间分桶，在每个桶内统计预测值 y_pred 的分布形态。
        如果同一真值区间内预测值呈现明显分叉（例如 std 很大且峰度显著为负），
        可能意味着：
          - 数据口径/标定混杂（同一风速对应两套不同的特征分布）；
          - 或模型回归收缩/欠拟合导致的偏移（通常表现为单峰但均值偏离）。

    说明：
        这里的“双峰”判断是经验规则，不是严格统计检验；其目的是快速发现异常线索。
        诊断结果会打印并写入 CSV，便于后续排查。
    """
    y_true = np.asarray(y_true, dtype=np.float64).reshape(-1)
    y_pred = np.asarray(y_pred, dtype=np.float64).reshape(-1)
    mask = np.isfinite(y_true) & np.isfinite(y_pred)
    ...
    for (lo, hi) in bins_to_check:
        m = (y_true >= lo) & (y_true < hi)
        preds_in_bin = y_pred[m]
        n = int(preds_in_bin.size)

        if n < 10:
            # 样本太少时统计不稳定，直接记录并跳过复杂指标
            rows.append({
                "lo": lo, "hi": hi, "n": n,
                "mean": float("nan"), "std": float("nan"),
                "skew": float("nan"), "kurtosis_excess": float("nan"),
                "flag_possible_bimodal": False,
            })
            print(f"  WS=[{lo},{hi}): n={n}，样本过少，跳过")
            continue

        mean_pred = float(np.mean(preds_in_bin))
        std_pred = float(np.std(preds_in_bin))

        if std_pred > 0:
            z = (preds_in_bin - mean_pred) / (std_pred + 1e-12)
        ...
        # 与原绘图版一致的“粗筛规则”：
        #   - 峰度很负：分布更“扁/双峰”的可能性增加
        #   - 且方差较大：说明同一真值区间内预测分布分散
        flag = bool((std_pred > 1.5) and (kurt_excess < -1.0))
        ...
        rows.append({
            ...
            "flag_possible_bimodal": flag,
        })

        tag = "⚠️ 可能双峰/口径混杂" if flag else "✅ 更像单峰/回归收缩"
        print(f"  WS=[{lo},{hi}): n={n:5d} | mean={mean_pred:.2f} | std={std_pred:.2f} | kurt={kurt_excess:.2f} -> {tag}")

    out_dir.mkdir(parents=True, exist_ok=True)
    out_csv = out_dir / "high_wind_bimodality_diagnosis.csv"
    pd.DataFrame(rows).to_csv(out_csv, index=False)
    print(f"  已保存高风速区间诊断统计: {out_csv}\n")
...
def run_training(cfg: TrainConfig) -> None:
    ...
    整体流程（高层逻辑）：
        1) 读取数据集 DataFrame（pkl）；
        2) 基于 df 构建辅助特征列（包含动态 SP 索引、log 变换等）；
        3) 按论文的“绝对时间”切分策略划分 train/val/test；
        4) 基于序列组构建滑动窗口样本（每个窗口长度 = time_steps）；
        5) 仅使用训练窗口涉及的观测点计算归一化统计量（避免数据泄漏）；
        6) 构建 Dataset/DataLoader（支持外部 DDM 存储与预加载）；
        7) 训练模型 + 早停；
        8) 可选：训练后执行 ReplaceMe 风格的连续层深度剪枝（减少推理成本）；
        9) 在验证/测试集上评估，并保存：指标 CSV、预测 NPZ、训练历史 CSV 等。

    注意：
        原脚本包含多种图表生成（学习曲线、散点密度、误差分布等）。
        按你的要求，这些绘图代码已全部移除，仅保留数值与文件输出。
    """
    ...
    # 注意：split_by_paper_time 是按【绝对时间】切分，可能会把同一个序列组切成两段。
    # 因此，部分滑动窗口会跨越 train/val/test 边界（窗口内索引不完全属于同一集合）。
    # 为避免训练/验证/测试相互污染，这里采用严格规则：
    #   - 只有当窗口内所有观测点都落在同一个集合时，才把该窗口分配给该集合；
    #   - 否则（跨集合窗口）直接丢弃。
    dropped_windows = 0
    for w in all_windows:
        ...
        if w_set.issubset(train_set):
            ...
            dropped_windows += 1
            # 跨集合窗口：丢弃（这是时间切分导致的正常现象，不代表 make_windows 有 bug）
            continue

    print(f"窗口分配: train={len(train_windows)}, val={len(val_windows)}, test={len(test_windows)}, dropped={dropped_windows}")

    # ==================== 回退处理 ====================
    if len(train_windows) == 0 or len(val_windows) == 0 or len(test_windows) == 0:
        print("⚠️ 警告: 按时间切分并过滤跨集合窗口后，某个集合窗口为空。回退到基于窗口的随机切分 (8:1:1)。")
...
# ========== 修改结束 ==========
    ...
    # ==================== 训练后诊断（无可视化） ====================
    # 说明：原脚本会生成多张图（学习曲线、KDE 散点密度、误差直方图等）。
    # 按你的要求，这里已移除所有绘图逻辑，仅保留数值/表格形式的诊断输出。

    # （可选）高风速窄区间诊断：用于快速排查“口径混杂/双峰分叉”迹象。
